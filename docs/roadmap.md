...
4. Idempotency: deterministic `externalId` per row (MP: route URL id + date + style; Kaya: full timestamp + gym + color + grade + ascent_type), unique constraint `(user, sourceApp, externalId)`, skip-and-count duplicates. **Known limit (2026-08-24): this only catches same-file re-imports.** Kaya itself ingests MP/8a.nu/Sendage ticks, so a user's Kaya export can contain ascents that originated elsewhere — the same real-world tick then arrives under two sourceApps with different externalIds, and this constraint is blind to it by design. Cross-source dedup is a Phase 3 problem (climb identity + same user/climb/date heuristic), not a tweak to this constraint.

   **Round-trip experiment findings (2026-08-31, `inputs/mtnprojecttokayatoexport.csv`, 795 MP rows → 759 Kaya rows):** no provenance marker of any kind; **location/country stripped on 100% of laundered rows** (MP breadcrumbs gone — cross-source matching has only climb name + date + grade to work with); ascent styles **flattened** — TR/Solo/Follow all became "Redpoint," so a laundered Kaya Redpoint is a default bucket, not a style claim, and the dedup matcher must never require style agreement; **Fell/Hung attempts dropped entirely** (23/24 vanished — Kaya logs ascents only); MP's −1 no-rating sentinel correctly became empty (no fabricated stars); duplicate same-climb sends became "Repeat"; dates survive with time zeroed. Consequences: direct-source imports are strictly richer than laundered ones (attempts, TR/solo distinction, and locations survive only direct) — on a dedup merge the direct row wins; eventually tell users to import each source directly rather than via Kaya's aggregate.
...
Detection decision (2026-08-24): auto-detect by header set, no source dropdown — a dropdown adds a wrong-pick failure mode while requiring the same header validation anyway. But surface the inference: the import result says "Detected: Kaya export." Manual override only if a genuinely ambiguous format ever ships.
...
Candidate sources beyond 8a (2026-08-24, from Kaya's own import menu — they ingest MP, 8a.nu, and Sendage, which both validates the aggregation market and causes the cross-source duplicate problem noted in Phase 1):
- **Sendage**: long-tail logbook app; needs a real export before any code.
- **Kaya's logbook template** (`inputs/logbook_template.csv`): header `Date,Send Type,Climb,Climb Type,Grade,Area,Country,Rating,Comments` after a 3-line preamble our current first-line-is-header parsing can't skip. Supporting it imports anyone who already formatted a spreadsheet for Kaya, and it's the prototype for publishing our own bring-your-own-spreadsheet template. Note: its `Climb Type` column resolves the Font-vs-French ambiguity that Kaya's own export can't.
...
- **iOS app (future, no phase yet)**: the standing rule is keep the API client-agnostic. One feature to not forget (2026-08-24): **shared links must open directly in the app** — text someone a TickList link and it opens the app, not the browser (Mountain Project fails this and it's infuriating). Mechanism: iOS Universal Links / Android App Links — an `apple-app-site-association` file served from the web domain maps URL paths to the app, so the *website's* URL design participates, not just the app. Consequence now: shareable pages need clean, stable paths (`/climbs/123`) that stay out of the API namespace — one more reason to settle N2 (the `/api` prefix) deliberately. Stack lean (2026-08-24): Android is also wanted, which makes React Native (+Expo) the default — one codebase for both stores, and the React skills/types from the web frontend transfer; Swift only wins if learning it is itself a goal.
- **Split the flat backend package** (decided 2026-08-24, ~40 files and growing): by feature (`imports/`, `ticks/`, `auth/`, `grades/`), never by layer (`controllers/`, `services/`); as its own commit between features — a package move touches every import statement and would bury real logic in the diff. Trigger: after Kaya merges, at latest when Phase 3's Climb/Area entities land. Update CLAUDE.md's "single flat package" line in the same commit.
